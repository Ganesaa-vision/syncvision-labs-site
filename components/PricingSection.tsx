  ...
  return (
    <section className="py-24 px-6 max-w-7xl mx-auto" id="pricing">
      <div className="text-center mb-20">
        <span className="font-mono text-indigo-400 text-xs font-bold uppercase tracking-[0.4em] mb-4 block">Investment</span>
        ...
        {plans.map((plan, idx) => (
          <div 
            ...
            className={`relative p-8 rounded-[2rem] border flex flex-col ${
              plan.isPopular 
                ? 'bg-slate-900/80 border-indigo-500/50 shadow-2xl shadow-indigo-500/10 scale-105 z-10' 
          ...
          >
            ...
            <div className="mb-8">
              ...
              <p className="text-slate-400 text-sm leading-relaxed min-h-[3rem]">{plan.description}</p>
            </div>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {plan.features.map((feature, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-slate-300">
                  <CheckCircle2 className={`w-5 h-5 flex-shrink-0 ${plan.isPopular ? 'text-indigo-400' : 'text-slate-600'}`} />
      ...
      <div className="p-8 md:p-12 bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden group">
        ...
        <div className="flex-1 relative z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-2xl font-bold text-white">Need a Custom Architecture?</h3>
          </div>
        ...
        <Link 
          to="/contact" 
          className="relative z-10 px-8 py-4 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-700 transition-all whitespace-nowrap border border-slate-700"
        >
          Get Custom Quote
        </Link>
      </div>
