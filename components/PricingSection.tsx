    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {plans.map((plan, index) => (
          <div 
            key={index}
            className={`relative p-6 lg:p-8 rounded-[2rem] border flex flex-col ${
              plan.isPopular 
          ...
          >
            ...
            <h3 className={`text-lg md:text-xl font-bold mb-2 ${plan.isPopular ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{plan.name}</h3>
            <div className="mb-4">
            ...
            <p className={`text-sm mb-6 md:mb-8 leading-relaxed ${plan.isPopular ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>{plan.description}</p>
