          ...
          {PRODUCTS.map((product, idx) => (
            <div key={product.id} className={`grid grid-cols-1 lg:grid-cols-12 gap-16 items-center ${idx % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
              <div className={`lg:col-span-7 ${idx % 2 === 1 ? 'lg:order-2' : ''}`}>
                <div className="relative rounded-[3rem] overflow-hidden border border-white/5 aspect-video group shadow-3xl bg-slate-900">
                   <img 
                   ...
                   <div className="absolute bottom-10 left-10 flex gap-4">
                      ...
                      <div className="px-5 py-2 bg-slate-900/60 backdrop-blur-md rounded-xl text-xs font-mono font-black text-slate-100 uppercase tracking-widest border border-white/10 shadow-xl">
                        {product.category}
                      </div>
                   </div>
                </div>
              ...
              <div className={`lg:col-span-5 ${idx % 2 === 1 ? 'lg:order-1' : ''}`}>
                 <div className="space-y-8">
                   <h3 className="text-4xl md:text-5xl font-mono font-black text-slate-100 uppercase tracking-tighter leading-none">{product.name}</h3>
                   <p className="text-interstellar font-mono text-sm font-black uppercase tracking-[0.3em]">{product.tagline}</p>
                   
                   <p className="text-slate-200 font-medium text-lg leading-relaxed border-l-4 border-interstellar/30 pl-8">
                     {product.longDescription || product.description}
                   </p>

                   <div className="grid grid-cols-1 gap-4 pt-4">
                     {product.features.map(f => (
                       <div key={f} className="flex items-center gap-4 text-xs font-mono font-black text-slate-100 uppercase tracking-widest p-4 bg-slate-900/50 border border-white/5 rounded-2xl group hover:border-indigo/30 transition-all">
